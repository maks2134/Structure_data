...
void removeUser() {
    int question;
    printf("Вы уверены, что готовы отменить бронь?\n");
    printf("1-Да, уверен(а)\n");
    printf("2-Нет, не уверен(а)\n");
    scanf("%d", &question);
    if (question == 1) {
        char ID[20];
        printf("Введите свой ID для подтверждения отмены брони: ");
        scanf("%s", ID);
        ...
        rename("C:\\Users\\maks2\\CLionProjects\\Structure_data\\temp.txt", "C:\\Users\\maks2\\CLionProjects\\Structure_data\\admin.txt");

        if (found) {
            printf("Бронь успешно отменена.\n");
        } else {
            printf("ID не найден.\n");
        }
    } else {
